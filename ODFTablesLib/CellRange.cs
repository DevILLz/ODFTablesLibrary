...
namespace ODFTablesLib
{
    public class CellRange : IEnumerable
    {
        ...
        private string RowNS = string.Empty;
        private string CellNS = string.Empty;
        private string Cell2NS = string.Empty;
        ...
        private string temp;
        public CellRange(XmlDocument doc, string temp = "", bool inner = false)
        {
            this.temp = temp;
            this.doc = doc;
        ...
        {
            RowNS = doc.Root.GetNamespaceOfPrefix("table").ToString();
            CellNS = RowNS;//doc.Root.GetNamespaceOfPrefix("office").ToString();
            Cell2NS = doc.Root.GetNamespaceOfPrefix("calcext").ToString();
        ...
        private void AddCell(int column, int rowSpan, int columnSpan, XmlNode child)
        {
            if (child.FirstChild?.Name.StartsWith("text") ?? false && column != -1)
            {
                ...
                if (rowSpan > 0 || columnSpan > 0)
                {
                    ...
                    var m = oldCell.MergedRange = new CellRange(doc, temp, true);
                    int i = 0, j = 0;
                    do
                    {
                        do
            ...
            {
                if (temp.EndsWith(".ods")) AddNode(child, column);
                else AddNewNode(child, column);
            }
        }
        ...
        private XmlNode AddNewNode(XmlNode node, int column, int row = -1)
        {
            ...
            if (row == -1) AddNode(node.PrependChild(t), column, 0, 0, row);
            else
            {
                XmlNode newRow = node;
                ...
                if (row > (Cells[Cells.Count - 1].Row + 1))
                    for (int i = 0; i < (row - Cells[Cells.Count - 1].Row) - 1; i++) // добавление пустых строк
                        newRow.AppendChild(doc.CreateElement("table", "table-row", RowNS));

                if (row > Cells[Cells.Count - 1].Row)
                    newRow = newRow.AppendChild(doc.CreateElement("table", "table-row", RowNS)); // добавление нужной строки
                else
                {
                    int count = -1;
                ...
                {
                    var cell = doc.CreateElement("table", "table-cell", $"{CellNS}"); // создать ячейку
                    var text = doc.CreateElement("text", "p", $"{TextNS}"); // создать текстовое поле
                    cell.AppendChild(text);
                    newRow.AppendChild(cell); 
                    AddNode(text, i, 0, 0, row);
                }
                var c = doc.CreateElement("table", "table-cell", $"{CellNS}");
                c.AppendChild(t);
        ...
        {
            CellRange range = new CellRange(doc, temp, true);
        ...
        {
            var range = new CellRange(doc, temp, true);
